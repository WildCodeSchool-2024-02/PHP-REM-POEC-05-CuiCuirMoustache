...
class AuthForRemoveController extends AbstractController
{
    ...
    public function editAccount()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (
                ...
                empty($data['firstname']) || empty($data['lastname']) || empty($data['phone'])
            ) {
                $errors[] = 'Tous les champs doivent être remplis.';
            } else {
                // Mettre à jour les informations de l'utilisateur
                $updated = $this->authModel->updateUser($user['id'], $data);
        ...
        try {
            echo $this->twig->render('Auth/edit_account.html.twig', [
                'user' => $user,
                'errors' => $errors
            ]);
        } catch (LoaderError | RuntimeError | SyntaxError $e) {
